...
Upload the generated artifacts from `dist\` with Twine:
...
- `analyze`: inspect debugger state, memory and stack dumps, deeper analysis passes, disassembly position, and xref analysis
...
- `search`: byte, asm, reference, xref, string, and GUID search helpers
